...
export default function Gallery() {
  ...
  const { items = [], total = 0, loading, error } = useGallery(
    PROPERTY_SLUG,
    { category: cat || "all", page: 1, limit: 24, enabled: !!cat }
  );
  ...
  const getAlt = (m) =>
    m.alt?.[lang] ||
    m.alt?.hu ||
    (lang === "en" ? "Gallery image" : lang === "de" ? "Galeriebild" : "Galéria kép");

  // Thumbok: videónál poszter vagy 1. mp képkocka, képnél standard transform
  const getThumb = (m) => {
  ...
  // ========== 2) LISTA NÉZET – kiválasztott kategória ==========
  return (
    <section id="gallery" className="scroll-mt-24 py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        {/* Fejléc + vissza a kategóriákhoz */}
                ...
                >
                  ...
                  {isVideo && (
                    <span className="absolute right-2 top-2 inline-flex items-center gap-1 rounded-full bg-black/60 px-2 py-1 text-xs text-white">
                      <FaPlay className="h-3 w-3" /> Video
                    </span>
                  )}
